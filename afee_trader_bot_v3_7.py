...
def load_bot_config() -> dict:
    """خواندن تنظیمات قابل ویرایش از bot_config.json (بدون ری‌استارت ربات)."""
    defaults = {
    ...
    if not os.path.exists(BOT_CONFIG_FILE):
        # اگر فایل وجود ندارد، با مقادیر پیش‌فرض یکی می‌سازیم
        try:
    ...
    try:
        ...
        with open(BOT_CONFIG_FILE, encoding="utf-8") as f:
            cfg = _json.load(f)
        # مطمئن می‌شویم همه کلیدها وجود دارند (backward-compat)
        for k, v in defaults.items():
            cfg.setdefault(k, v)
        return cfg
    except Exception as e:
        log.warning(f"bot_config.json خراب بود، مقادیر پیش‌فرض استفاده می‌شود: {e}")
        return defaults
...
def get_dynamic_score_threshold(regime_label: str, runtime_min: int) -> int:
    """
    ...
    elif "HIGH" in regime_label or "WHIPSAW" in regime_label:
...
def init_database():
    ...
    try:
        ...
        """)
        # ── signal_notifications: ذخیره message_id تلگرام و وضعیت اعلان‌های TP/SL ──
        conn.execute("""
...
def mark_notification_sent(signal_id: int, event: str, trade_closed: bool = False):
    """علامت‌گذاری ارسال اعلان برای جلوگیری از ارسال مجدد. event: 'tp1'|'tp2'|'sl'"""
    col_map = {"tp1": "tp1_notified", "tp2": "tp2_notified", "sl": "sl_notified"}
...
async def update_trade_outcomes(session: aiohttp.ClientSession):
    ...
    async with _db_lock:
        ...
        for row in open_rows:
            ...
            try:
                ...
                if new_fail_count >= MAX_PRICE_FETCH_FAILURES:
                    ...
                    log.warning(
                        f"signal_id={row['id']} ({row['symbol']}): {new_fail_count} consecutive "
                        f"price-fetch failures — force-closing as EXPIRED (price_unavailable)"
                    )
                    updates.append((
                        row["id"], "EXPIRED", entered_at, tp1_hit, tp2_hit, None, None,
            ...
            if new_status and new_status != status:
                updates.append((
                    ...
                    0, None  # FIX (bug audit): price fetch succeeded → reset fail counter, no close_reason
                ))
        ...
        # ── ارسال reply اعلان‌های TP1/TP2/SL به پیام اصلی سیگنال ──
        for u in updates:
            signal_id  = u[0]
            new_status = u[1]
            final_pnl  = u[9]
            # یافتن اطلاعات سیگنال (symbol و direction) از open_rows
            row_info = next((r for r in open_rows if r["id"] == signal_id), None)
            if row_info is None:
                continue
            symbol    = row_info["symbol"]
            direction = row_info["direction"]
            old_status = row_info["status"]
            tp1_was_hit = row_info["tp1_hit"]

            try:
                if new_status == "TP1_HIT":
                    # final_pnl is None at TP1_HIT (trade not yet closed); calculate TP1 profit
                    tp1_pnl = ((row_info["tp1"] - row_info["entry"]) / row_info["entry"] * 100) \
                              if direction == "BUY" \
                              else ((row_info["entry"] - row_info["tp1"]) / row_info["entry"] * 100)
                    await send_outcome_reply(session, signal_id, symbol, direction, "tp1", round(tp1_pnl, 2))
                elif new_status == "TP2_HIT":
                    await send_outcome_reply(session, signal_id, symbol, direction, "tp2", final_pnl)
                elif new_status == "SL_HIT":
                    await send_outcome_reply(session, signal_id, symbol, direction, "sl_before_tp1", final_pnl)
                elif new_status == "TP1_TOUCHSL":
                    # TP1 زده شد سپس SL → اعلان SL after TP1
                    await send_outcome_reply(session, signal_id, symbol, direction, "sl_after_tp1", final_pnl)
            except Exception as e:
...
def get_trades_for_report(trades: list, report_date_str: str) -> list:
    ...
    """
    # FIX (bug audit): فیلتر فقط بر اساس raw_status انجام می‌شود، نه compat status.
    # دلیل: compat_status برای TP1_TOUCHSL برابر "TP1" است که قبلاً اشتباهاً در
    # OPEN_STATUSES قرار داشت و باعث می‌شد TP1_TOUCHSL (یک معامله بسته‌شده) از
    # گزارش روزانه حذف شود. raw_status تنها منبع تصمیم برای باز/بسته بودن است.
...
def build_daily_report(trades: list, report_date_str: str) -> str:
    ...
    FIX v3.5 (Patch #1): تمام آمار از یک لیست واحد (displayed_trades) محاسبه می‌شود.
    MISSED، PENDING، ENTERED، TP1_HIT (هنوز باز) هرگز نه نمایش داده می‌شوند نه شمارش.
    """
    todays_raw = get_trades_for_report(trades, report_date_str)
    ...
    STATUS_FA = {
        ...
        "TP1_TOUCHSL": "TP1 خورد، سپس Break Even (Touch SL) 🔄",
    ...
    # FIX (bug audit): MISSED دیگر از گزارش روزانه حذف نمی‌شود — یک معامله بسته‌شده
    # است (raw_status=MISSED, closed_at ست شده) و باید در تعداد کل سیگنال‌ها بیاید.
    # فقط در محاسبه وین‌ریت شرکت نمی‌کند (نه win نه loss) — به همین دلیل در
    # WIN_STATUSES/LOSS_STATUSES پایین قرار ندارد.
    displayed_trades = list(todays_raw)

    if not displayed_trades:
        return (
    ...
    # ── FIX v3.5: نمایش و شمارش از همان لیست displayed_trades ──
    for t in displayed_trades:
        raw_st = t.get("raw_status", t.get("status", ""))
        pnl    = t.get("pnl_percent", 0) or 0
        total_pnl += pnl
        sign      = "🟢+" if pnl >= 0 else "🔴"
        status_fa = STATUS_FA.get(raw_st, STATUS_FA.get(t.get("status", ""), raw_st))
        opened_date = (t.get("opened_at") or "")[:10]
        date_note   = f" | صدور: {opened_date}" if opened_date != report_date_str else ""
        lines.append(
            f"💎 {t['symbol']} ({t['strategy']}) — {sign}{abs(pnl):.2f}% — {status_fa}{date_note}"
        )
        if raw_st in CLOSED_STATUSES or t.get("status", "") in CLOSED_STATUSES:
            closed_count += 1
            if raw_st in WIN_STATUSES or t.get("status", "") in WIN_STATUSES:
                win_count += 1
    ...
    # FIX v3.5: تعداد = دقیقاً همان تعدادی که نمایش داده شد (displayed_trades)
    lines.append(f"📦 <b>تعداد کل سیگنال‌های بسته‌شده:</b> {len(displayed_trades)}")
...
# ─── TELEGRAM ─────────────────────────────────────────────────────────────────
async def send_telegram(session: aiohttp.ClientSession, text: str, chat_id: str = None, reply_to_message_id: int = None) -> Optional[int]:
...
async def broadcast_signal(session: aiohttp.ClientSession, text: str, state: dict) -> Optional[int]:
    ...
    if main_chat_entry is None or main_chat_entry.get("active", True):
        main_message_id = await send_telegram(session, text, TELEGRAM_CHAT_ID)
...
async def send_outcome_reply(session: aiohttp.ClientSession, signal_id: int, symbol: str,
    ...
    """
    ارسال reply به پیام اصلی سیگنال هنگام رسیدن به TP1، TP2، یا Stop Loss.
    event: 'tp1' | 'tp2' | 'sl_after_tp1' | 'sl_before_tp1'
    """
    ...
    try:
        ...
        trade_closed = event in ("tp2", "sl_after_tp1", "sl_before_tp1")
        event_key = "tp1" if event == "tp1" else ("tp2" if event == "tp2" else "sl")
...
def build_message(
    ...
    tp2_line = ""
    if tp2_unlikely:
        tp2_line = f"⚠️ <b>TP2 Potential پایین</b> — فقط TP1 هدف‌گیری شود\n"
    ...
    for tf in ["1m", "3m", "5m"]:
        ...
        if swept_up:
            ...
            if lower_high and broke_low:
                ...
                if not (c[-1]["close"] < c[-1]["open"]):
                    continue
                shadow_candle_high = max(highs[-10:-1])
                stop  = calc_safe_stop(c, "SELL", closes[-1])  # Patch #13
                entry = closes[-1]
        ...
        if swept_down:
            ...
            if higher_low and broke_high:
                ...
                if not (c[-1]["close"] > c[-1]["open"]):
                    continue
                stop  = calc_safe_stop(c, "BUY", closes[-1])  # Patch #13
                entry = closes[-1]
...
async def _hammer_fib_on_tf(
    ...
    for tf in entry_tfs:
        ...
        if direction == "SELL":
            ...
            if not broke: continue
            # Patch #3a: Close Confirmation
            if not has_close_confirmation(lc, lc[-1]["close"], "BUY"):
                continue
            wave_low  = min(lows[-20:])
            wave_high = max(highs[-30:-20])
            fib_618   = wave_low + (wave_high - wave_low) * 0.618
            stop      = calc_safe_stop(lc, "BUY", fib_618)  # Patch #13
            entry     = fib_618
            tp1, tp2  = calc_targets(entry, stop, "BUY")
    ...
    for tf in ["5m", "15m"]:
        ...
        if trend_up:
            ...
            if last["close"] < last["open"]:  # bearish candle
                # Patch #3a: Close Confirmation
                if not has_close_confirmation(c, price, "SELL"):
                    continue
                mid   = (last["high"] + last["low"]) / 2
                stop  = calc_safe_stop(c, "SELL", mid)  # Patch #13
                entry = mid
                tp1, tp2 = calc_targets(entry, stop, "SELL")
            ...
            if last["close"] > last["open"]:  # bullish candle
                # Patch #3a: Close Confirmation
                if not has_close_confirmation(c, price, "BUY"):
                    continue
                mid   = (last["high"] + last["low"]) / 2
                stop  = calc_safe_stop(c, "BUY", mid)  # Patch #13
                entry = mid
                tp1, tp2 = calc_targets(entry, stop, "BUY")
                score = calc_score([strong, near is not None, True], sr_score=sr_sc)
                return {
        ...
        if trend_up:
            ...
            if cond1 and cond2 and cond3:
                ...
                if not pb_valid:
                    ...
                    continue
                # Patch #3a: Close Confirmation
                if not has_close_confirmation(c, c[-1]["close"], "BUY"):
                    continue
                fib_low   = t1["open"]
                fib_high  = max(t2["high"], t3["high"])
                fib_618   = fib_low + (fib_high - fib_low) * 0.618
                stop  = calc_safe_stop(c, "BUY", fib_618)  # Patch #13
                entry = fib_618
                tp1, tp2 = calc_targets(entry, stop, "BUY")
                base_score = calc_score([cond1, cond2, cond3, near in levels], sr_score=sr_sc_tf)
                score = max(1, min(100, round(base_score * pb_score_mult)))
...
async def strategy_exhaustion(
    ...
    if trend_up and price < near * 1.001:
        ...
        if (body_weakening or vol_weakening) and last_weak:
            # Patch #3a: Close Confirmation
            if not has_close_confirmation(c15, price, "SELL"):
                return None
        ...
        if (body_weakening or vol_weakening) and last_weak:
            # Patch #3a: Close Confirmation
            if not has_close_confirmation(c15, price, "BUY"):
                return None
...
async def run_live_filters(session, symbol: str, result: dict, state: dict) -> tuple[bool, list[str]]:
    ...
    if not user_is_strict:
        dynamic_min = get_dynamic_score_threshold(regime_label, runtime_min)
    else:
...
async def _run_backtest_job(session, job: dict) -> dict:
    ...
    # FIX (bug audit): فرمول وین‌ریت یکسان با calc_strategy_stats — فقط معاملات
    # قطعی (TP/SL) در مخرج هستند، نه کل معاملات (که شامل MISSED/EXPIRED هم می‌شود).
...
async def handle_update(session, update, state):
    ...
    if "message" in update:
        ...
        if text.startswith("/bl ") and has_permission(uid, "blacklist", state):
            ...
            # /config key value  — تغییر مستقیم bot_config.json از تلگرام
            # مثال: /config TOP_N_COINS 50
            #        /config DYNAMIC_SCORE_THRESHOLD_TREND 80
            parts_cfg = text.split(None, 2)
            if len(parts_cfg) < 3:
                cfg_now = load_bot_config()
                cfg_lines = ["⚙️ <b>bot_config.json (تنظیمات قابل ویرایش)</b>", ""]
                for k, v in cfg_now.items():
                    cfg_lines.append(f"• <code>{k}</code> = <b>{v}</b>")
                cfg_lines.append("")
                cfg_lines.append("برای تغییر: /config KEY VALUE")
                cfg_lines.append("مثال: /config TOP_N_COINS 50")
                await send_msg(session, cid, "\n".join(cfg_lines))
            else:
                ...
                if cfg_key not in valid_keys:
                    await send_msg(session, cid,
                        f"❌ کلید نامعتبر: <code>{cfg_key}</code>\n"
                    ...
                    try:
                        ...
                        with open(BOT_CONFIG_FILE, "w", encoding="utf-8") as _cf:
                            import json as _json2
                            _json2.dump(cfg_now, _cf, indent=2, ensure_ascii=False)
                        await send_msg(session, cid,
                            f"✅ <code>{cfg_key}</code> → <b>{cfg_val}</b> ذخیره شد در bot_config.json\n"
                            f"تغییر فوری است — ربات را ری‌استارت نکنید.")
                        log.info(f"bot_config: {cfg_key}={cfg_val} set by admin {uid}")
                    except (ValueError, TypeError):
                        await send_msg(session, cid, f"❌ مقدار نامعتبر: {cfg_val_str}. باید عدد باشد.")
                    except Exception as e:
                        await send_msg(session, cid, f"❌ خطا در ذخیره: {e}")
        ...
        if data == "main_menu":
            ...
            await edit_msg(session, cid, mid,
                ...
                f"به‌جای انتخاب صرف بر اساس حجم معاملات، ابتدا {_qr_pool} نماد برتر بر اساس حجم انتخاب می‌شوند؛ "
                f"سپس برای همان نامزدها امتیاز کیفیت محاسبه و فقط {_qr_top_n} نماد با بالاترین کیفیت برای اسکن نهایی انتخاب می‌شوند.\n\n"
            ...
            await edit_msg(session, cid, mid, f"⏳ در حال محاسبه کیفیت {_qr_pool2} نماد... (ممکن است کمی طول بکشد)")
            try:
        ...
        # ── مدیریت ادمین‌ها ──
            ...
            await edit_msg(session, cid, mid,
                f"⚙️ <b>تنظیمات</b>\n\n"
                f"🔍 ارزها (TOP_N_COINS): <b>{_top_n_now}</b> (از bot_config.json)\n"
                ...
                f"💡 برای تغییر TOP_N_COINS یا آستانه‌های Dynamic Score، فایل <b>bot_config.json</b> را ویرایش کنید.",
                reply_markup={"inline_keyboard": [[{"text": "◀️ برگشت", "callback_data": "main_menu"}]]})
...
async def scan_once(session, state):
    ...
    for res in all_results:
        ...
        for result in res:
            ...
            if backtest_on:
                try:
                    signal_id = log_trade(result)
                except Exception as e:
                    log.error(f"log_trade error: {e}")
            tg_message_id = await broadcast_signal(session, build_message(
