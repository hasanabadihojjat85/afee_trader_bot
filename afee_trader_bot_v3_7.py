...
def build_daily_report(trades: list, report_date_str: str) -> str:
    ...
    # ── FIX v3.5: یک لیست واحد — فقط معاملاتی که نمایش می‌دهیم ──
    # MISSED اصلاً در get_trades_for_report می‌آید ولی با raw_status=MISSED؛
    # اینجا هم دوباره حذف می‌کنیم تا هماهنگی کامل داشته باشیم.
    displayed_trades = []
    for t in todays_raw:
        raw_st = t.get("raw_status", t.get("status", ""))
        if raw_st == "MISSED":
            continue
        displayed_trades.append(t)
